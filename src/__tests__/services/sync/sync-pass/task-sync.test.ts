...
describe('TaskSync creation and title sync', () => {
  ...
  it('creates a Todoist task for a line that has no anchor yet', async () => {
    ...
    expect(created).toEqual([
      { title: 'Buy milk', projectId: PROJECT, description: `TaskBridge ID: ^${blockId}`, labels: [], isCompleted: false },
    ]);
  });
  ...
  it('bakes the device tag into a freshly minted block id', async () => {
    ...
    expect(note.content).toMatch(/^- \[ \] Buy milk \^tb-dev1a-[a-z0-9]{8}$/);
  });
  ...
  it('lets the Obsidian edit win when both sides changed to different titles, and counts the conflict', async () => {
    const note = new FakeNote('- [ ] Local wins ^tb-a1');
    const links = new TaskLinkStore([
      { blockId: 'tb-a1', providerTaskId: TASK_ID, lastSyncedTitle: 'Original' },
    ]);
    const pushed: string[] = [];
    const sync = makeSync(note, links, {
    ...
    expect(note.content).toBe('- [ ] Local wins ^tb-a1');
  });
  ...
  it('pulls the remote title when it is the newer of two conflicting edits', async () => {
    ...
    expect(note.content).toBe('- [ ] Remote wins ^tb-a1');
    expect(links.get('tb-a1')?.lastSyncedTitle).toBe('Remote wins');
  });
  ...
  it('is not a conflict when both sides changed to the same title', async () => {
    const note = new FakeNote('- [ ] Same title ^tb-a1');
    const links = new TaskLinkStore([
      { blockId: 'tb-a1', providerTaskId: TASK_ID, lastSyncedTitle: 'Original' },
    ]);
    const sync = makeSync(note, links, {
    ...
    expect(note.content).toBe('- [ ] Same title ^tb-a1');
    expect(links.get('tb-a1')?.lastSyncedTitle).toBe('Same title');
  });
